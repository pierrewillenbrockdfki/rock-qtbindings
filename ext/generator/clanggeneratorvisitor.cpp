...
#if 0
void ClangGeneratorVisitor::visitDeclarator(DeclaratorAST* node)
{
    if (createTypeAlias) {
        createType = false;
        createTypedef = false;
    ...
    if (createTypedef) {
        ...
        Class* parent = klass.isEmpty() ? 0 : scopes.top().klass;
    ...
    if (node->parameter_declaration_clause && !inMethod && inClass) {
        ...
        bool isConstructor = (declName == scopes.top().klass->name());
        bool isDestructor = (declName == "~" + scopes.top().klass->name());
        Type* returnType = currentTypeRef;
        if (isConstructor) {
            // constructors return a pointer to the class they create
            Type t(scopes.top().klass); t.setPointerDepth(1);
        ...
        currentMethod = Method(scopes.top().klass, declName, returnType, access.top());
    ...
    if (!inMethod && !klass.isEmpty() && inClass) {
        Field field = Field(scopes.top().klass, declName, currentTypeRef, access.top());
        if (isStatic) field.setFlag(Field::Static);
        scopes.top().klass->appendField(field);
        return;
...
void ClangGeneratorVisitor::visitElaboratedTypeSpecifier(ElaboratedTypeSpecifierAST* node)
{
    tc->run(node);
    createType = true;
...
void ClangGeneratorVisitor::visitEnumSpecifier(EnumSpecifierAST* node)
{
    nc->run(node->name);
    Class* parent = klass.isEmpty() ? 0 : scopes.top().klass;
...
void ClangGeneratorVisitor::visitEnumerator(EnumeratorAST* node)
{
...
void ClangGeneratorVisitor::visitInitializerClause(InitializerClauseAST *)
{
    // we don't care about initializers
    return;
}

void ClangGeneratorVisitor::visitParameterDeclaration(ParameterDeclarationAST* node)
{
...
void ClangGeneratorVisitor::visitSimpleDeclaration(SimpleDeclarationAST* node)
{
    bool popKlass = false;
        ...
        do {
            ...
            if (it->element && it->element->declarator && it->element->declarator->parameter_declaration_clause) {
                if (popKlass) {
                    ...
                    QString oldClass = scopes.top().klass->toString();
                    scopes.top().klass->setParent(0);
                    scopes.top().klass->setNameSpace(QString());
                    classes.insert(scopes.top().klass->toString(), *scopes.top().klass);
    ...
    if (node->storage_specifiers) {
        ...
        do {
            if (it->element && m_session->token_stream->kind(it->element) == Token_static) {
                ...
                if (popKlass)
                    klass.pop();
                isStatic = isVirtual = isPure = false;
                return;
            }
            it = it->next;
        } while (end != it);
...
void ClangGeneratorVisitor::visitSimpleTypeSpecifier(SimpleTypeSpecifierAST* node)
{
...
void ClangGeneratorVisitor::visitTypedef(TypedefAST* node)
{
    createTypedef = true;
...
void ClangGeneratorVisitor::visitTypeAlias(TypeAliasAST* node)
{
...
void ClangGeneratorVisitor::visitUsing(UsingAST* node)
{
    nc->run(node->name);
    if (inClass) {
        ...
        Q_FOREACH (const Method& meth, clazz->methods()) {
            if (meth.name() != methodName)
                continue;
            
            // prevent importing of already overridden (pure) virtuals
            if (scopes.top().klass->methods().contains(meth))
                continue;
            
            Method copy = meth;
            copy.setDeclaringType(scopes.top().klass);
            copy.setAccess(access.top());
            scopes.top().klass->appendMethod(copy);
        }
    } else {
