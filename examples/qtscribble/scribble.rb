...
require 'Qt5'
	...
	class ScribbleArea < Qt5::Widget
		...
		def slotClearArea()
		  ...
		  #
		  # This method does the actual loading. It relies on Qt5::Pixmap (and the
		  # underlying I/O machinery) to determine the filetype.
		  #
		def slotLoad( filename )
			if !@_buffer.load( filename )
				Qt5::MessageBox.warning( nil, "Load error", "Could not load file" )
			end
		...
		def slotSave( filename )
			if !@_buffer.save( filename, "BMP" )
				Qt5::MessageBox.warning( nil, "Save error", "Could not save file" )
			end
		end
		...
		def mousePressEvent(event)
			if event.button() == RightButton
				@_popupmenu.exec( Qt5::Cursor.pos() )
			else
		...
		def mouseMoveEvent(event)
			# create a Qt5::Painter object for drawing onto the window
			windowpainter = Qt5::Painter.new()
			# and another Qt5::Painter object for drawing int an off-screen pixmap
			bufferpainter = Qt5::Painter.new()
		...
		def resizeEvent(event)
			save = Qt5::Pixmap.new( @_buffer )
			@_buffer = save.scaled(event.size.width, event.size.height)
			@_buffer.fill( Qt5::Color.new(Qt5::white) )
			drawPixmap( @_buffer, 0, 0, save )
		end
	end

class ScribbleWindow < Qt5::Widget
	...
	def initialize()
		...
		@_menubar = Qt5::MenuBar.new( self )  # create a menu bar
...
#		Qt5::Object.connect( @_colormenu, SIGNAL( "activated( int )" ),
#						 self, SLOT( "slotColorMenu( int )" ) )
		...
		@_helpmenu.addAction( "&About Qt", self, SLOT( "slotAboutQt()" ) )
		 ...
		 # We create a Qt5::ScrollView and a ScribbleArea. The ScribbleArea will
		 # be managed by the scroll view.#
		@_scrollview = Qt5::ScrollArea.new( self )
...
#		@_scrollview.addChild( @_scribblearea )
		Qt5::Object.connect( self, SIGNAL( "colorChanged(QColor)" ),
						 @_scribblearea, SLOT( "setColor(QColor)" ) )
		Qt5::Object.connect( self, SIGNAL( "save(const QString&)" ),
						 @_scribblearea, SLOT( "slotSave(const QString&)" ) )
		Qt5::Object.connect( self, SIGNAL( "load(const QString&)" ),
	...
	def slotAbout()
		Qt5::MessageBox.information( self, "About QtScribble 5",
										"This is the Scribble 5 application\n" +
										"Copyright 1998 by Mathias Kalle Dalheimer\n")
	end
	
	def slotAboutQt()
		Qt5::MessageBox.aboutQt( self, "About Qt" )
	end
	...
	def slotColorMenu( item )
	  ...
	  # Qt5::FileDialog to ask the user for a filename, then emits a save()
	...
	def slotLoad()
		 ...
		 #
		filename = Qt5::FileDialog.getOpenFileName( ".", "*.bmp", self )
		if !filename.nil?
			emit load( filename )
		end
	end
	...
	def slotSave()
		 ...
		 # current directory, the filter *.bmp.
		 #
		filename = Qt5::FileDialog.getSaveFileName( ".", "*.bmp", self )
		if !filename.nil?
			emit save( filename )
		end
	end
...
myapp = Qt5::Application.new(ARGV)
